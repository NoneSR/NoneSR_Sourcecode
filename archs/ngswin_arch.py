...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        """
        B_, N, C = x.shape
        qkv = self.qkv(x).reshape(B_, N, 3, self.num_heads, -1).permute(2, 0, 3, 1,
                                                                        4)  # [qkv(3), B*wh*ww, nheads, WH*WW, dim_per_head]
...
class NSTB(nn.Module):  # N-Gram Swin Transformer Block
    ...
    def _attention(self, x, num_patches):
        ...
        reversed_x = torch.roll(shifted_x, shifts=(self.shift_size, self.shift_size),
                                dims=(1, 2)) if self.shift_size > 0 else shifted_x  # [B, ph, pw, D]
...
class PatchMerging(nn.Module):
    ...
    def flops(self, resolutions):
        ...
        # reduction
        flops += (H // 2) * (
                    W // 2) * 4 * self.dim * self.downsample_dim + self.downsample_dim  # linear.weight, linear.bias
        return flops
...
class EncoderLayer(nn.Module):
    ...
    def flops(self, resolutions):
        ...
        flops += H * W * self.num_cas * self.dim * self.dim + self.dim if isinstance(self.across_cascade_proj,
                                                                                     nn.Linear) else 0
...
class Reconstruction(nn.Sequential):
    def __init__(self, target_mode, dim, out_chans=3):
        ...
        if target_mode in ['light_x2', 'light_x3', 'light_x4']:
            ...
            self.add_module('shuffler',
                            nn.PixelShuffle(upscale))  # [B, dim/(upscale^2), upscale*H, upscale*W)
    ...
    def flops(self, resolutions):
        ...
        if self.target_mode in ['light_x2', 'light_x3', 'light_x4']:
            ...
            flops += H * W * 3 * 3 * self.dim * self.out_chans * (self.upscale ** 2) + H * W * self.out_chans * (
                        self.upscale ** 2)
            # to_origin: conv.weight, conv.bias
            flops += (self.upscale * H) * (self.upscale * W) * 3 * 3 * self.out_chans * self.out_chans + (
                        self.upscale * H) * (self.upscale * W) * self.out_chans
        return flops
...
class NGswin(nn.Module):
    ...
    def __init__(
        ...
        dpr = [x.item() for x in
               torch.linspace(0, drop_path_rate, sum(depths) + sum(dec_depths))]  # stochastic depth decay rule
    ...
    def forward_encoder(self, x):
        ...
        e1_, e1, num_patches1 = self.encoder_layer1(c0,
                                                    num_patches0)  # [B, HW, D], [B, HW/(2^2), D], (H/2, W/2). encoder-stage1

        c1 = torch.cat([self.inter_pool(c0, num_patches0), e1], dim=-1)  # [B, HW/(2^2), 2D], (H, W). ASPC
        e2_, e2, num_patches2 = self.encoder_layer2(c1,
                                                    num_patches1)  # [B, HW/(2^2), D], [B, HW/(4^2), D], (H/4, W/4). encoder-stage2

        c2 = torch.cat([self.inter_pool(c1, num_patches1), e2], dim=-1)  # [B, HW/(4^2), 3D], (H, W). ASPC
        e3_, e3, num_patches3 = self.encoder_layer3(c2,
                                                    num_patches2)  # [B, HW/(4^2), D], [B, HW/(8^2), D], (H/8, W/8). encoder-stage3
...
if __name__ == '__main__':
    ...
    data = torch.randn(1,3,640, 320).cuda()
    t1 = time.time()
    net(data)
    t2 = time.time()
    print(t2-t1)