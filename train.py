...
def train_pipeline(root_path):
    ...
    log_file = osp.join(opt['path']['log'], f"train_{opt['name']}_{get_time_str()}.log") #函数用于将以下组件串联成一个单一的路径字符串
    logger = get_root_logger(logger_name='basicsr', log_level=logging.INFO, log_file=log_file)
    logger.info(get_env_info())#写log
    #logger.info(dict2str(opt)) #删去了yml文件
    # initialize wandb and tb loggers
    tb_logger = init_tb_loggers(opt)

    # create train and validation dataloaders
    result = create_train_val_dataloader(opt, logger) #把opt里面的train、val提取出来
    train_loader, train_sampler, val_loaders, total_epochs, total_iters = result

    # create model
    model = build_model(opt)
    if opt['train']['print_net']:
        model.print_network(model.net_g)  # 打印网络
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        while train_data is not None:
            ...
            if current_iter > total_iters:
                break
            # update learning rate warmup训练开始时，学习率从一个小值逐渐增加到所需的值。
            model.update_learning_rate(current_iter, warmup_iter=opt['train'].get('warmup_iter', -1))
            # training
            model.feed_data(train_data) #把数据放在GPU上
            model.optimize_parameters(current_iter) #优化过程
            iter_timer.record()
            if current_iter == 1:
